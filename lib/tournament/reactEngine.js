...
const TV_SCREEN_MODES = ['auto', 'live', 'standings', 'info', 'lanes', 'rotating', 'final'];
const TV_SCREEN_THEMES = ['classic', 'highContrast', 'light', 'minimal'];

function createDefaultTvScreens() {
  return {
    screen1: {
      label: 'Skærm 1',
      mode: 'live',
...
export function createDefaultTournamentState() {
  return {
    ...
    tvScreens: createDefaultTvScreens(),
    lastGenerationError: ''
  };
}
...
export function normalizeTournamentState(input) {
  ...
  return {
    ...
    roundPublished: typeof state.roundPublished === 'boolean'
    ...
    matches: Array.isArray(state.matches) ? normalizeLaneQueues(state.matches.map(normalizeMatch).filter(Boolean)) : [],
    ...
    tvScreens: normalizeTvScreens(state.tvScreens, laneCount),
...
export function updateTournamentMaxLosses(state, maxLosses) {
  const nextMaxLosses = clampNumber(maxLosses, 1, 20, state.maxLosses);
  if (state.finalResults) {
    return { ...state, lastGenerationError: 'Nederlagsgrænsen kan ikke ændres efter turneringen er afsluttet.' };
  }
  if (state.started && !canManageEntriesBetweenRounds(state)) {
    return { ...state, lastGenerationError: 'Nederlagsgrænsen kan kun ændres mellem runder.' };
  }
  const updateEntry = entry => {
    const eliminated = Number(entry.losses) >= nextMaxLosses;
    return {
      ...entry,
      active: !eliminated,
      eliminationRound: eliminated ? (entry.eliminationRound ?? (state.currentRound || 1)) : null
    };
  };
  return {
    ...state,
    maxLosses: nextMaxLosses,
    players: state.players.map(updateEntry),
    fixedTeams: state.fixedTeams.map(updateEntry),
    lastGenerationError: ''
  };
}
...
export function setActiveLane(state, laneNumber, active) {
  ...
  const matches = !active
    ? redistributeLaneQueue(state.matches, lane, activeLanes)
    : normalizeLaneQueues(state.matches);
  return {
    ...state,
    activeLanes,
    matches,
...
export function publishRound(state) {
  if (!state.matches.length && !getSkippedEntries(state).length) {
    return { ...state, lastGenerationError: 'Der er ingen genereret runde at offentliggøre.' };
  }
  const invalidMatch = state.matches.find(match => (
    !Number.isInteger(match.laneNumber)
    || !state.activeLanes.includes(match.laneNumber)
    || !Number.isInteger(match.lanePosition)
  ));
  if (invalidMatch) {
    return { ...state, lastGenerationError: 'Alle kampe skal være tildelt en aktiv bane, før runden kan offentliggøres.' };
  }
  return { ...state, roundPublished: true, spectatorOverride: null, lastGenerationError: '' };
}

export function hidePublishedRound(state) {
  if (state.matches.some(match => match.winner)) {
    return { ...state, lastGenerationError: 'Runden kan ikke skjules, efter en vinder er markeret.' };
  }
  return { ...state, roundPublished: false, spectatorOverride: null, lastGenerationError: '' };
}
...
export function assignMatchLane(state, matchId, laneNumber) {
  ...
  const matchToMove = state.matches.find(match => match.id === matchId);
  if (!matchToMove) return state;
  const targetPosition = normalizedLane
    ? Math.max(0, ...state.matches
      .filter(match => match.id !== matchId && match.laneNumber === normalizedLane)
      .map(match => Number(match.lanePosition) || 0)) + 1
    : null;
  return {
    ...state,
    matches: normalizeLaneQueues(state.matches.map(match => match.id === matchId
      ? { ...match, laneNumber: normalizedLane, lanePosition: targetPosition }
      : match)),
    lastGenerationError: ''
  };
}

export function moveMatchInLaneQueue(state, matchId, direction) {
  const match = state.matches.find(item => item.id === matchId);
  if (!match || !Number.isInteger(match.laneNumber)) return state;
  const queue = state.matches
    .filter(item => item.laneNumber === match.laneNumber)
    .sort(compareLaneQueueMatches);
  ...
  if (match.winner || target.winner) {
    return { ...state, lastGenerationError: 'Afsluttede kampe kan ikke flyttes i banekøen.' };
  }
  const nextPositions = new Map(queue.map((item, queueIndex) => [item.id, queueIndex + 1]));
  nextPositions.set(match.id, targetIndex + 1);
  nextPositions.set(target.id, index + 1);
  return {
    ...state,
    matches: normalizeLaneQueues(state.matches.map(item => item.laneNumber === match.laneNumber
      ? { ...item, lanePosition: nextPositions.get(item.id) }
      : item)),
    lastGenerationError: ''
  };
}

export function getMatchLaneStatus(state, matchId) {
  const match = state.matches.find(item => item.id === matchId);
  if (!match) return 'unassigned';
  if (match.winner) return 'completed';
  if (!Number.isInteger(match.laneNumber)) return 'unassigned';
  const hasEarlierUnfinishedMatch = state.matches.some(item => (
    item.id !== match.id
    && item.laneNumber === match.laneNumber
    && !item.winner
    && compareLaneQueueMatches(item, match) < 0
  ));
  return hasEarlierUnfinishedMatch ? 'queued' : 'current';
}

export function selectWinner(state, matchId, winner) {
  const match = state.matches.find(item => item.id === matchId);
  if (!match) return state;
  if (!state.roundPublished) {
    return { ...state, lastGenerationError: 'Offentliggør runden, før du markerer vindere.' };
  }
  if (!match.winner && getMatchLaneStatus(state, matchId) === 'queued') {
    return { ...state, lastGenerationError: 'Kampen står i kø. Afslut den tidligere kamp på banen først.' };
  }
  return {
    ...state,
...
export function completeRound(state) {
  ...
  const history = {
    ...
    matches: state.matches.map(match => ({
      ...
      lanePosition: Number.isInteger(match.lanePosition) ? match.lanePosition : null,
...
export function updateTvScreen(state, screenKey, patch = {}) {
  const key = screenKey === 'screen2' ? 'screen2' : 'screen1';
  const tvScreens = normalizeTvScreens(state.tvScreens, state.laneCount);
  return {
    ...state,
    tvScreens: {
      ...tvScreens,
      [key]: normalizeTvScreen({ ...tvScreens[key], ...patch }, key, state.laneCount)
    },
    lastGenerationError: ''
  };
}
...
function assignLanes(state) {
  ...
  return {
    ...
    matches: state.matches.map((match, index) => ({
      ...match,
      laneNumber: lanes[index % lanes.length],
      lanePosition: Math.floor(index / lanes.length) + 1
    }))
  };
}

function compareLaneQueueMatches(left, right) {
  const leftPosition = Number.isInteger(Number(left.lanePosition)) ? Number(left.lanePosition) : Number.MAX_SAFE_INTEGER;
  const rightPosition = Number.isInteger(Number(right.lanePosition)) ? Number(right.lanePosition) : Number.MAX_SAFE_INTEGER;
  return leftPosition - rightPosition || Number(left.id) - Number(right.id);
}

function normalizeLaneQueues(matches) {
  const positions = new Map();
  const lanes = [...new Set(matches.map(match => match.laneNumber).filter(Number.isInteger))];
  lanes.forEach(lane => {
    matches
      .filter(match => match.laneNumber === lane)
      .sort(compareLaneQueueMatches)
      .forEach((match, index) => positions.set(match.id, index + 1));
  });
  return matches.map(match => ({
    ...match,
    lanePosition: Number.isInteger(match.laneNumber) ? positions.get(match.id) : null
  }));
}

function redistributeLaneQueue(matches, disabledLane, activeLanes) {
  if (!activeLanes.length) return normalizeLaneQueues(matches);
  const displaced = matches
    .filter(match => match.laneNumber === disabledLane && !match.winner)
    .sort(compareLaneQueueMatches);
  let nextMatches = matches.map(match => match.laneNumber === disabledLane && !match.winner
    ? { ...match, laneNumber: null, lanePosition: null }
    : match);
  displaced.forEach(displacedMatch => {
    const targetLane = [...activeLanes].sort((left, right) => {
      const leftCount = nextMatches.filter(match => match.laneNumber === left && !match.winner).length;
      const rightCount = nextMatches.filter(match => match.laneNumber === right && !match.winner).length;
      return leftCount - rightCount || left - right;
    })[0];
    const targetPosition = Math.max(0, ...nextMatches
      .filter(match => match.laneNumber === targetLane)
      .map(match => Number(match.lanePosition) || 0)) + 1;
    nextMatches = nextMatches.map(match => match.id === displacedMatch.id
      ? { ...match, laneNumber: targetLane, lanePosition: targetPosition }
      : match);
  });
  return normalizeLaneQueues(nextMatches);
}
...
function normalizeMatch(match) {
  ...
  if (match.mode === 'fixed') {
    return {
      ...
      lanePosition: Number.isInteger(Number(match.lanePosition)) && Number(match.lanePosition) > 0 ? Number(match.lanePosition) : null
    };
  }
  return {
    id: Number(match.id),
...
function normalizeTvScreens(value, laneCount) {
  const defaults = createDefaultTvScreens();
  return {
    screen1: normalizeTvScreen({ ...defaults.screen1, ...(value?.screen1 || {}) }, 'screen1', laneCount),
    screen2: normalizeTvScreen({ ...defaults.screen2, ...(value?.screen2 || {}) }, 'screen2', laneCount)
  };
}

function normalizeTvScreen(value, key, laneCount) {
  const mode = TV_SCREEN_MODES.includes(value?.mode) ? value.mode : (key === 'screen2' ? 'standings' : 'live');
  const theme = TV_SCREEN_THEMES.includes(value?.theme) ? value.theme : 'classic';
  return {
    label: String(value?.label || (key === 'screen2' ? 'Skærm 2' : 'Skærm 1')).trim().slice(0, 40),
    mode,
    rowsPerPage: clampNumber(value?.rowsPerPage, 6, 20, 12),
    matchesPerPage: clampNumber(value?.matchesPerPage, 1, 20, 6),
    rotationSeconds: clampNumber(value?.rotationSeconds, 5, 60, 10),
    showQr: value?.showQr !== false,
    announcement: String(value?.announcement || '').trim().slice(0, 180),
    hideHeader: !!value?.hideHeader,
    autoHideHeader: !!value?.autoHideHeader,
    theme,
    lanes: Array.isArray(value?.lanes)
      ? [...new Set(value.lanes.map(Number).filter(lane => Number.isInteger(lane) && lane >= 1 && lane <= laneCount))]
      : []
  };
}
