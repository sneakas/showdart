...
export function togglePlayerTag(state, playerId, tag) {
  if (state.teammateMode === 'fixed' || !['S', 'O'].includes(tag)) return state;
  return {
    ...state,
...
export function setActiveLane(state, laneNumber, active) {
  ...
  return { ...state, activeLanes: [...activeSet].sort((a, b) => a - b), lastGenerationError: '' };
}

export function generateMatches(state) {
  if (!state.started || state.finalResults) return state;
  if (state.pendingTagChanges && state.teammateMode !== 'fixed') {
    return { ...state, lastGenerationError: 'Godkend tag ændringer før næste runde genereres.' };
  }

  let working = state.teammateMode === 'fixed' ? state : resetCompletedRoleCycles(state);
...
function generateFixedMatches(state, activeTeams) {
  ...
  for (let attempt = 0; attempt < MAX_SHUFFLE_ATTEMPTS; attempt++) {
    ...
    const penalty = getFixedRoundFairnessPenalty(state, pairs) + skipPenalty;
...
function assignAutomaticTags(state) {
  if (state.teammateMode === 'fixed') {
    return {
      ...state,
      pendingTagChanges: false,
      fixedTeams: state.fixedTeams.map(team => state.skippedTeamIds.includes(team.id) ? { ...team, lastSkippedRound: state.currentRound } : team)
    };
  }
