...
export function buildScreenState(state) {
  ...
  const roundPublished = typeof state?.roundPublished === 'boolean' ? state.roundPublished : true;
    ...
    ? state.matches.map(match => {
        if ((match.mode === 'fixed' || isFixedTeams) && Number.isInteger(match.team1TeamId) && Number.isInteger(match.team2TeamId)) {
          ...
          return {
            ...
            lanePosition: Number.isFinite(Number(match.lanePosition)) ? Number(match.lanePosition) : null,
  ...
  const matchesWithQueueStatus = matches.map(match => {
    if (match.winner) return { ...match, queueStatus: 'completed', displayQueuePosition: null };
    if (!Number.isInteger(match.laneNumber)) return { ...match, queueStatus: 'unassigned', displayQueuePosition: null };
    const queue = matches
      .filter(item => item.laneNumber === match.laneNumber)
      .sort((left, right) => (Number(left.lanePosition) || Number.MAX_SAFE_INTEGER) - (Number(right.lanePosition) || Number.MAX_SAFE_INTEGER) || left.id - right.id);
    const unfinishedQueue = queue.filter(item => !item.winner);
    const queueIndex = unfinishedQueue.findIndex(item => item.id === match.id);
    return {
      ...match,
      queueStatus: queueIndex === 0 ? 'current' : 'queued',
      displayQueuePosition: queueIndex >= 0 ? queueIndex + 1 : null
    };
  });

  const sortedMatches = [...matchesWithQueueStatus].sort((left, right) => {
    const laneDiff = getLaneSortValue(left.laneNumber) - getLaneSortValue(right.laneNumber);
    if (laneDiff !== 0) {
      return laneDiff;
    }

    const queueDiff = (Number(left.lanePosition) || Number.MAX_SAFE_INTEGER) - (Number(right.lanePosition) || Number.MAX_SAFE_INTEGER);
  ...
  const visibleMatches = roundPublished ? sortedMatches : [];
  const visibleSkippedEntries = roundPublished ? skippedEntries : [];
  ...
  if (finalResults.length > 0) {
    phase = 'final';
  } else if (state?.started && (visibleMatches.length > 0 || visibleSkippedEntries.length > 0)) {
    phase = 'round';
  ...
  return {
    ...
    skippedEntries: visibleSkippedEntries,
    finalResults,
    finalPlacements,
    ...
    tvScreens: state?.tvScreens && typeof state.tvScreens === 'object' ? state.tvScreens : {},
