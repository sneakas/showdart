...
export function buildScreenState(state) {
  ...
  const matchesWithQueueStatus = matches.map(match => {
    if (match.winner) return { ...match, queueStatus: 'completed' };
    if (!Number.isInteger(match.laneNumber)) return { ...match, queueStatus: 'unassigned' };
    const queue = matches
    ...
    const firstUnfinished = queue.find(item => !item.winner);
    return { ...match, queueStatus: firstUnfinished?.id === match.id ? 'current' : 'queued' };
  });
