...
const CHAMPIONSHIP_SCREEN_MODES = ['auto', 'registration', 'matches', 'initialStandings', 'aStandings', 'bStandings', 'aBracket', 'bBracket', 'brackets', 'podium', 'announcement'];

function createDefaultChampionshipScreens() {
  const base = {
    mode: 'auto', rowsPerPage: 12, matchesPerPage: 8, groupsPerPage: 2, rotationSeconds: 10,
    announcement: '', hideHeader: false, hideHero: false, hideConnection: false,
    hidePageIndicator: false, hideLaneInfo: false, hideFooter: false, lanes: []
  };
  return {
    screen1: { ...base, label: 'Skærm 1' },
    screen2: { ...base, label: 'Skærm 2' }
  };
}
...
export function createDefaultChampionshipState() {
  return {
    ...
    publicScreens: createDefaultChampionshipScreens(),
    lastError: ''
  };
}
...
export function normalizeChampionshipState(input) {
  ...
  return {
    ...
    publicScreens: normalizeChampionshipScreens(state.publicScreens, config.laneCount),
    lastError: typeof state.lastError === 'string' ? state.lastError : ''
  };
}

export function updateChampionshipPublicScreen(state, screenKey, patch = {}) {
  const key = screenKey === 'screen2' ? 'screen2' : 'screen1';
  const screens = normalizeChampionshipScreens(state.publicScreens, state.config.laneCount);
  return {
    ...state,
    publicScreens: {
      ...screens,
      [key]: normalizeChampionshipScreen({ ...screens[key], ...patch }, key, state.config.laneCount)
    },
    lastError: ''
  };
}
...
function normalizeChampionshipScreens(value, laneCount) {
  const defaults = createDefaultChampionshipScreens();
  return {
    screen1: normalizeChampionshipScreen({ ...defaults.screen1, ...(value?.screen1 || {}) }, 'screen1', laneCount),
    screen2: normalizeChampionshipScreen({ ...defaults.screen2, ...(value?.screen2 || {}) }, 'screen2', laneCount)
  };
}

function normalizeChampionshipScreen(value, key, laneCount) {
  return {
    label: String(value?.label || (key === 'screen2' ? 'Skærm 2' : 'Skærm 1')).trim().slice(0, 40),
    mode: CHAMPIONSHIP_SCREEN_MODES.includes(value?.mode) ? value.mode : 'auto',
    rowsPerPage: clamp(value?.rowsPerPage, 4, 24, 12),
    matchesPerPage: clamp(value?.matchesPerPage, 1, 24, 8),
    groupsPerPage: clamp(value?.groupsPerPage, 1, 6, 2),
    rotationSeconds: clamp(value?.rotationSeconds, 5, 120, 10),
    announcement: String(value?.announcement || '').trim().slice(0, 240),
    hideHeader: !!value?.hideHeader,
    hideHero: !!value?.hideHero,
    hideConnection: !!value?.hideConnection,
    hidePageIndicator: !!value?.hidePageIndicator,
    hideLaneInfo: !!value?.hideLaneInfo,
