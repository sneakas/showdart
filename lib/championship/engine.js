...
export function getGroupStandings(state, groupId) {
  ...
  return stats
    .sort((left, right) => right.points - left.points || right.miniWins - left.miniWins || right.tieBreakWins - left.tieBreakWins || left.seed - right.seed || left.teamId - right.teamId)
    .map((item, index, all) => ({
      ...item,
      rank: index + 1,
      unresolvedTie: all.some(other => other.teamId !== item.teamId && other.points === item.points && other.miniWins === item.miniWins && other.tieBreakWins === item.tieBreakWins)
    }));
}
...
export function getChampionshipMatchStatus(state, matchId) {
  ...
  if (match.winnerId || match.isBye || match.voided) return 'completed';
  if (!Number.isInteger(match.laneNumber)) return 'unassigned';
  const active = getCurrentMatches(state).filter(item => item.laneNumber === match.laneNumber && !item.winnerId && !item.isBye && !item.voided);
...
export function moveChampionshipMatchInQueue(state, matchId, direction) {
  ...
  if (match.winnerId || target.winnerId) return { ...state, lastError: 'Afsluttede kampe kan ikke flyttes.' };
...
export function setChampionshipActiveLane(state, laneNumber, active) {
  ...
  let matches = state.matches.map(match => !active && state.activeMatchIds.includes(match.id) && match.laneNumber === lane
    ...
    : match);
  const displaced = matches.filter(match => state.activeMatchIds.includes(match.id) && !match.winnerId && !match.isBye && !match.voided && !match.laneNumber);
...
export function hideChampionshipRound(state) {
  if (getCurrentMatches(state).some(match => match.winnerId)) return { ...state, lastError: 'Runden kan ikke skjules efter et resultat er registreret.' };
...
export function completeChampionshipRound(state) {
  const current = getCurrentMatches(state).filter(match => !match.isBye && !match.voided);
  if (!current.length || current.some(match => !match.winnerId)) return { ...state, lastError: 'Alle aktuelle kampe skal have en vinder.' };
...
export function generateRequiredTieBreaks(state) {
  ...
  const ties = groups.flatMap(group => {
    ...
    return getBoundaryTieTeamIds(state, group.id, count).length > 1
      ? [{ group, teamIds: getBoundaryTieTeamIds(state, group.id, count) }]
      : [];
  });
  ...
  ties.forEach(({ group, teamIds }) => {
    roundRobin(teamIds).forEach((roundMatches, roundIndex) => {
      roundMatches.forEach(([team1Id, team2Id]) => {
        const match = {
          id: id++, stage: 'tiebreak', groupId: group.id, division: group.division,
          round: firstTieRound + roundIndex, team1Id, team2Id,
          winnerId: null, isTieBreak: true, tieBreakCycle: Date.now(), laneNumber: null, lanePosition: null,
          published: false, voided: false, isBye: false, matchType: 'tiebreak'
        };
        matches.push(match);
...
function getBoundaryTieTeamIds(state, groupId, qualifierCount) {
  ...
  if (index < 0 || index >= standings.length - 1) return [];
  const boundary = standings[index];
  const same = standings.filter(item => item.points === boundary.points && item.miniWins === boundary.miniWins && item.tieBreakWins === boundary.tieBreakWins);
  const positions = same.map(item => standings.findIndex(entry => entry.teamId === item.teamId));
  return Math.min(...positions) <= index && Math.max(...positions) > index ? same.map(item => item.teamId) : [];
}
...
function assignLanesToMatches(matches, matchIds, activeLanes) {
  ...
  return matches.map(match => {
    if (!ids.has(match.id) || match.isBye || match.voided || match.winnerId) return match;
...
function normalizeMatch(match) {
  ...
  return {
    ...
    voided: !!match.voided, isBye: !!match.isBye, isTieBreak: !!match.isTieBreak
  };
}
